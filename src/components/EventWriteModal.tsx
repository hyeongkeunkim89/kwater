...
export function EventWriteModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(j.error ?? "이벤트 등록에 실패했습니다.");
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/55 backdrop-blur-sm animate-hero-caption-in">
      <div className="w-full max-w-lg rounded-2xl bg-white border border-slate-200 p-6 shadow-2xl flex flex-col max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <h3 className="text-lg font-black text-slate-800">✍️ 새 이벤트/프로그램 등록</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4 text-sm">
          ...
          {/* 제목 */}
          <div className="flex flex-col gap-1.5">
            <label className="font-bold text-slate-700">이벤트 제목</label>
            <input
              type="text"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              placeholder="이벤트 또는 체험 프로그램 제목을 입력해 주세요."
          ...
          {/* 본문 내용 */}
          <div className="flex flex-col gap-1.5">
            <label className="font-bold text-slate-700">이벤트 내용</label>
            <textarea
              rows={5}
              value={content}
              onChange={(e) => setContent(e.target.value)}
              placeholder="이벤트 요강 및 상세 일정을 입력해 주세요."
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="font-bold text-slate-700">시작 날짜</label>
              <input
                type="date"
                value={startDate}
                onChange={(e) => setStartDate(e.target.value)}
                className="w-full min-h-11 rounded-xl border border-slate-200 px-3 font-semibold text-slate-700 focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500"
              />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="font-bold text-slate-700">종료 날짜</label>
              <input
                type="date"
                value={endDate}
          ...
          <div className="flex flex-col gap-1.5">
            <label className="font-bold text-slate-700 flex items-center gap-1.5">
              ...
              <span className="text-[10px] font-medium text-slate-400">배너용 사진 URL 기입 가능</span>
            </label>
            <input
              type="url"
              value={imageUrl}
